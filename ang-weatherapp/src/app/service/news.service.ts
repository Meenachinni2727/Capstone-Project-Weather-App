 ...
 // API_KEY = "8ef68669556f47d0bb7689c1c4d88786";
  ...
  initCategory(){
   ...
   public addToFavourite(favourite: News): Observable<News> {
    ...
    //,{headers : tokenHeader}
    return this.http.post<News>('http://localhost:50496/api/Favourite',News);
  }
  public getFavourites(userId : any): Observable<News[]> {
    //var tokenHeader=new HttpHeaders({'Authorization':'Bearer '+localStorage.getItem('token')});
    //,{headers : tokenHeader}
    return this.http.get<News[]>('http://localhost:50496/api/Favourite/'+userId );
  }
  public myfavourites(){
    //var tokenHeader=new HttpHeaders({'Authorization':'Bearer '+localStorage.getItem('token')});
    //,{headers : tokenHeader}
    return this.http.get('http://localhost:50496/api/Favourite');
  }

  deleteFavourite(id,title):Observable<any>{
    // var tokenHeader=new HttpHeaders({'Authorization':'Bearer '+localStorage.getItem('token')});
   // ,{headers : tokenHeader}
    return this.http.delete<any>('http://localhost:50496/api/Favourite/'+id+'/'+title);
  }
  ...
  public getRecommends(){
    return this.http.get('http://localhost:62242/api/Recommends');
  }
  public addRecommend(rec:News): Observable<News>{
    return this.http.post<News>('http://localhost:62242/api/Recommends',rec);
  }
}
